...
impl Packager for OsxPkgPackager {
    ...
    fn build(&self, ctx: &BuildContext) -> Result<PathBuf> {
        ...
        let _ = super::apply_contents_full(cfg.config(), ctx.staging_root, "osxpkg")?;
        ...
        if let Some(body) = super::render_script_body(cfg.config(), job, &cfg.scripts().preinstall)?
        {
            scripts.push(("preinstall".to_string(), body.into_bytes()));
            script_names.push("preinstall");
        }
        if let Some(body) =
            super::render_script_body(cfg.config(), job, &cfg.scripts().postinstall)?
        {
