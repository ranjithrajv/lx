...
pub struct BitbucketSourcePlugin;

impl SourcePlugin for BitbucketSourcePlugin {
    ...
    fn latest_release(
        ...
        let client = lpt_lib::source_client::new_client_for::<lpt_lib::bitbucket::BitbucketClient>(
            token, cache_dir,
        )?;
    ...
    fn raw_get(&self, url: &str, token: Option<&str>) -> Result<Box<dyn std::io::Read + Send>> {
        let client = lpt_lib::bitbucket::BitbucketClient::new(token.map(|s| s.to_string()))?;
        client.raw_get(url)
    }
}
...
pub fn parse_bitbucket_url(s: &str) -> Option<String> {
    ...
    if let Some(repo) = try_parse_for_host(s, lpt_lib::constants::DEFAULT_BITBUCKET_HOST) {
        return Some(repo);
    }
    ...
    if let Ok(host) = std::env::var("BITBUCKET_HOST") {
        let host = host
            ...
            .trim_end_matches('/');
        if !host.is_empty() && host != lpt_lib::constants::DEFAULT_BITBUCKET_HOST {
