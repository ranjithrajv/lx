...
impl SourcePlugin for ForgejoSourcePlugin {
    ...
    fn latest_release(
        ...
        let client = lpt_lib::source_client::new_client_for::<lpt_lib::forgejo::ForgejoClient>(
            token, cache_dir,
        )?;
    ...
    fn raw_get(&self, url: &str, token: Option<&str>) -> Result<Box<dyn std::io::Read + Send>> {
        let client = lpt_lib::forgejo::ForgejoClient::new(token.map(|s| s.to_string()))?;
        client.raw_get(url)
    }
}

pub fn parse_forgejo_url(s: &str) -> Option<String> {
    if let Some(repo) = try_parse_for_host(s, lpt_lib::constants::DEFAULT_FORGEJO_HOST) {
        return Some(repo);
    }
    ...
    if let Ok(host) = std::env::var("FORGEJO_HOST") {
        let host = host
            ...
            .trim_end_matches('/');
        if !host.is_empty() && host != lpt_lib::constants::DEFAULT_FORGEJO_HOST {
