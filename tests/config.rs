...
fn maintainer_env_overrides_builtin_default() {
    ...
    // $LPT_MAINTAINER > built-in latest-debs default.
    let unset = resolve_default_maintainer(std::env::var("NO_SUCH_VAR_XYZ_123").ok());
    assert_eq!(unset, lpt_lib::constants::DEFAULT_MAINTAINER);
    assert_eq!(
    ...
    assert_eq!(
        ...
        lpt_lib::constants::DEFAULT_MAINTAINER
    );
}
...
"#;
    ...
    assert_eq!(
        patterns.len(),
        lpt_lib::constants::DEFAULT_ARCHITECTURES.len()
    );
    assert_eq!(
        patterns["amd64"].release_pattern,
...
fn filter_expired_distributions_drops_bullseye_after_lts_end() {
    use lpt_lib::config::filter_expired_distributions;
