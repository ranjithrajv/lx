...
Verified via `stage_install_tree_flat_mode_installs_man_pages_and_license` (synthetic binary dir with a `.1` man page and a `LICENSE` file, asserting both land at the correct FHS paths) plus `man_section_recognizes_plain_and_gzipped_pages` and `is_license_like_matches_common_names_case_insensitively`. Not additionally re-verified against a real GitHub release in this pass — checked `eza-community/eza`'s actual release tarball (`tar -tzf`) and confirmed it ships only `./eza` at the top level, so it can't exercise this path; the unit test exercises the exact same `stage_install_tree`/`stage_ancillary_file` code a real build calls, so a live release with no matching files to find would add nothing this test doesn't already show.
