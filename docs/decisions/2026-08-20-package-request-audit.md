...
**Found, not fixed (separate, pre-existing issue, out of scope):** every repo under the `latest-debs` GitHub org is named `<pkg>-debian` (e.g. `eza-debian`), but `find_asset`/`control_version` expect the bare `<pkg>` name embedded in the asset filename to equal the `package` argument -- i.e. the repo name and the expected package argument don't actually match today. Discovered while trying to verify this change end-to-end via a real `lpt install eza-debian`: it fails with "no .deb for amd64/bookworm" regardless of verification flags, because `find_asset` is filtering on the wrong prefix. This predates today's change (confirmed by reasoning through `find_asset`'s unchanged matching logic) and affects `lpt install`/`lpt upgrade` generally, not just the new verification behavior. Left unfixed since it's an unrelated bug, not a trust/verifiability gap -- flagged here so it isn't lost.
...
- **`debs.rs` addendum verified live too**: downloaded a real `latest-debs/eza-debian` release asset (no sidecar published) via a temporary test calling `verify_sidecar_or_require_flag` directly against the real URL (worked around the `find_asset` naming-mismatch bug above, which blocks going through `lpt install` end-to-end) -- confirmed it fails with `allow_unverified: false` and succeeds with `allow_unverified: true`, then removed the scratch test.
...
2027-08-20 -- revisit the 4th recommendation (signature/attestation verification) if a concrete upstream that publishes Sigstore bundles or GitHub attestations comes up. Fix the `find_asset` repo-name/package-name mismatch noted in the addendum above so `lpt install`/`lpt upgrade` can be verified end-to-end without a workaround.
